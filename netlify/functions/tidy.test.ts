...
vi.mock("../../src/tidy/githubIo", () => ({
  tidyPageDirect: vi.fn(async ({ id }: { id: string }) => ({ id, title: "Tidied" })),
}));
...
describe("tidy function", () => {
  ...
  it("returns the saved page after Claude tidy", async () => {
    const { handler } = await import("./tidy");
    const { tidyPageDirect } = await import("../../src/tidy/githubIo");
    const response = await handler(event() as never, {} as never);
    expect(response.statusCode).toBe(200);
    expect(JSON.parse(response.body ?? "{}")).toMatchObject({ id: "page_hub_p" });
    expect(tidyPageDirect).toHaveBeenCalledWith(
